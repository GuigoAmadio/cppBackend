...
bool TenantRepository::createTenant(const std::string& id, const std::string& name, const std::string& subdomain) {
    try {
        auto conn = pool_->acquire();
        ...
        auto result = conn->executeParams(sql, {id, name, subdomain});
        
        if (result.isSuccess()) {
            LOG_INFO("Tenant created (simple): " + name + " (" + subdomain + ")");
            return true;
        }
        
        LOG_ERROR("Failed to create tenant: " + name);
        return false;
    } catch (const std::exception& e) {
        LOG_ERROR("Exception creating tenant: " + std::string(e.what()));
        return false;
    }
}

bool TenantRepository::userHasAccessToTenant(const std::string& userId, const std::string& tenantId) {
    LOG_DEBUG("🔍 [TENANT] userHasAccessToTenant() called");
    ...
    bool hasAccess = result.isSuccess() && result.rowCount() > 0;
    LOG_DEBUG("   Query success: " + std::string(result.isSuccess() ? "TRUE" : "FALSE"));
    LOG_DEBUG("   Row count: " + std::to_string(result.rowCount()));
    LOG_DEBUG("   Result: " + std::string(hasAccess ? "TRUE (HAS ACCESS)" : "FALSE (NO ACCESS)"));
